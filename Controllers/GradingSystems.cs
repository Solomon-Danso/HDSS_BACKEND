...
public async Task<IActionResult> ReportAnalysis(string level, string SID, string year, string term){
...
var counter = context.ReportAnalysis.Where(a=>a.StudentId==analysis.StudentId).Count();
if(counter>0){
var c = context.ReportAnalysis.FirstOrDefault(a=>a.StudentId==analysis.StudentId);
if(c!=null){


c.ThisTermTotalScoreObtained = analysis.ThisTermTotalScoreObtained;
c.ThisTermEntireTotalScore = analysis.ThisTermEntireTotalScore;
c. ThisTermAverageScore = analysis. ThisTermAverageScore;
c.ThisTermTotalPass = analysis.ThisTermTotalPass;
c.ThisTermTotalFailed = analysis.ThisTermTotalFailed;
c.StudentId = analysis.StudentId;
c.ClassName = analysis.ClassName;
c.StudentName = analysis.StudentName;
c.ThisTermAcademicTerm = analysis.ThisTermAcademicTerm;
c.ThisTermAcademicYear = analysis.ThisTermAcademicYear;
c.DateAdded =  DateTime.Today.Date.ToString("dd MMMM, yyyy");
...
var termResults = context.ReportAnalysis
    .Where(a => a.ClassName == analysis.ClassName && a.ThisTermAcademicYear == analysis.ThisTermAcademicYear && a.ThisTermAcademicTerm == analysis.ThisTermAcademicTerm)
    .ToList();
